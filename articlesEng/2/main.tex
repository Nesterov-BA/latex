\documentclass[leqno]{article}
\usepackage{amsmath,amsfonts,amssymb,amsthm,indentfirst,enumerate,textcomp}
\usepackage[T1,T2A]{fontenc}
\usepackage[utf8]{inputenc}
\usepackage{arxiv}
\usepackage[english]{babel}
\usepackage{indentfirst, array, csquotes}
\usepackage{amscd,latexsym}
\usepackage{mathrsfs}
%\usepackage[ruled, linesnumbered]{algorithm2e}
\usepackage{tabularx}
\usepackage{multirow}

\usepackage{graphicx}
\usepackage{textcase}% Оформление страниц
\usepackage[style = ieee]{biblatex}
\addbibresource{refs.bib}
%
\begin{document}
\title{Angles of clouds and the Gromov--Hausdorff distance between
  clouds with different angles
}
\author{B.\,A.~Nesterov}
\date{}
\maketitle
\begin{abstract}
  In the present paper the Gromov--Hausdorff distance between clouds
  of metric spaces is studied.  The new concept of the angle of a cloud is introduced and it's properties are studied. With this notion of the angle we prove that if two clouds have different angles and their stationary groups intersect non-trivially, then the Gromov-Hausdorff distance between them is necessarily infinite.
\end{abstract}
\section{Introduction}
\include{intro}
\end{document}
