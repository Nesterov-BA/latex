...
\end{proof}
\begin{remark}
  Теорема \ref{thm:diffAndDist} не является обобщением теоремы
  \ref{thrmDist}. Так, пространства \( \Z \) и \( \widetilde \R \) из
  теоремы \ref{thrmRUltraMetric} позволяют доказать, что расстояние
  между \( [\Delta _1] \) и \( [\R] \) равно бесконечности
  используя теорему  \ref{thrmDist}. При этом, угол между этими
  пространствами может быть меньше \( \frac \pi 2 \), то есть они не
  доказывают, что углы облаков \( [\Delta _1] \) и \( [\R] \) различны. Поэтому,
  используя только эти пространства и теорему  \ref{thm:diffAndDist}
  нельзя доказать бесконечность расстояния.
\end{remark}
